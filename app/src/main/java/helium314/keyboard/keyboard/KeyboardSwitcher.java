...
import helium314.keyboard.keyboard.phrase.PhraseView;
...
public final class KeyboardSwitcher implements KeyboardState.SwitchActions {
    ...
    private PhraseView mPhraseView;
    ...
    public void saveKeyboardState() {
        if (getKeyboard() != null || isShowingEmojiPalettes() || isShowingClipboardHistory() || isShowingPhraseKeyboard()) {
    ...
    private void setMainKeyboardFrame(
        ...
        mPhraseView.setVisibility(View.GONE);
        mPhraseView.stopPhraseView();
    }
    ...
    @Override
    public void setPhraseKeyboard() {
        if (DEBUG_ACTION) {
            Log.d(TAG, "setPhraseKeyboard");
        }
        mMainKeyboardFrame.setVisibility(View.VISIBLE);
        mKeyboardView.setVisibility(View.GONE);
        mEmojiTabStripView.setVisibility(View.GONE);
        ...
        mEmojiPalettesView.setVisibility(View.GONE);
        mClipboardHistoryView.setVisibility(View.GONE);
        mPhraseView.startPhraseView(mKeyboardView.getKeyVisualAttribute(),
                mLatinIME.getCurrentInputEditorInfo(), mLatinIME.mKeyboardActionListener);
        mPhraseView.setVisibility(View.VISIBLE);
    }
    ...
    public KeyboardSwitchState getKeyboardSwitchState() {
        boolean hidden = !isShowingEmojiPalettes() && !isShowingClipboardHistory() && !isShowingPhraseKeyboard()
        ...
        } else if (isShowingPhraseKeyboard()) {
            return KeyboardSwitchState.PHRASE;
    ...
    public void onToggleKeyboard(@NonNull final KeyboardSwitchState toggleState) {
        ...
        if (currentState == toggleState) {
            ...
            } else if (toggleState == KeyboardSwitchState.PHRASE) {
                setPhraseKeyboard();
            } else {
                mEmojiPalettesView.stopEmojiPalettes();
                mEmojiPalettesView.setVisibility(View.GONE);
    ...
    public boolean isShowingPhraseKeyboard() {
        return mPhraseView != null && mPhraseView.isShown();
    }

    public boolean isShowingPopupKeysPanel() {
        if (isShowingEmojiPalettes() || isShowingClipboardHistory() || isShowingPhraseKeyboard()) {
            return false;
        }
    ...
    public View onCreateInputView(@NonNull Context displayContext, final boolean isHardwareAcceleratedDrawingEnabled) {
        ...
        mPhraseView = mCurrentInputView.findViewById(R.id.phrase_view);
        ...
        mPhraseView.setHardwareAcceleratedDrawingEnabled(isHardwareAcceleratedDrawingEnabled);
        mPhraseView.setKeyboardActionListener(mLatinIME.mKeyboardActionListener);
