...
class KeyboardState(private val switchActions: SwitchActions) {
    ...
    private fun setPhraseKeyboard() {
        ...
        prevMainKeyboardWasShiftLocked = alphabetShiftState.isShiftLocked
        alphabetShiftState.setShiftLocked(false)
        switchActions.setPhraseKeyboard()
    }
    ...
    fun onEvent(event: Event, autoCapsFlags: Int, recapitalizeMode: RecapitalizeMode?) {
        ...
        when (switchState) {
            ...
            SwitchState.SYMBOL_BEGIN ->
                if (mode == Mode.EMOJI || mode == Mode.CLIPBOARD || mode == Mode.PHRASE) {
                    // When in the Emoji, clipboard or phrase keyboard, we don't want to switch back to the main layout even
        ...
        if (Constants.isLetterCode(code)) {
            ...
            KeyCode.PHRASE -> setPhraseKeyboard()
